...
from hhat_lang.core.function_resolver import (
...
def create_dummy_hat_file(
    base_path: Path, module_path_str: str, content: str = "", wrap_main: bool = True
):
...
def test_locate_function_in_main_hat(project_structure):
    file_path, func_name = locate_function_source(
        "main", "main_function", str(project_structure)
    )
...
def test_locate_function_in_src_folder(project_structure):
    file_path, func_name = locate_function_source(
        "physics.kinematics", "calculate_speed", str(project_structure)
    )
...
def test_locate_function_in_nested_folder(project_structure):
    file_path, func_name = locate_function_source(
        "ml.models.linear", "linear_regression", str(project_structure)
    )
...
def test_invalid_function_name_special_chars(project_structure):
    with pytest.raises(
        FunctionResolutionError, match="Function name 'my@func' is invalid"
    ):
...
def test_invalid_function_name_starts_with_number(project_structure):
    with pytest.raises(
        FunctionResolutionError, match="Function name '1func' is invalid"
    ):
...
def test_invalid_module_path_component_special_chars(project_structure):
    with pytest.raises(
        FunctionResolutionError, match="Path component 'my@dir' is invalid"
    ):
...
def test_invalid_module_path_component_starts_with_number(project_structure):
    ...
    with pytest.raises(
        FunctionResolutionError, match="Path component '1dir' is invalid"
    ):
...
def test_invalid_module_path_hat_prefix_in_src(project_structure):
    ...
    create_dummy_hat_file(
        project_structure / "src", "hat_forbidden.test", "func test(){}"
    )
    with pytest.raises(
        FunctionResolutionError,
        match="Directory component 'hat_forbidden' within 'src/' cannot start with 'hat_'",
    ):
...
def test_non_existent_file_in_src(project_structure):
    with pytest.raises(FunctionResolutionError, match="Source file not found"):
        locate_function_source(
            "nonexistent.module", "some_func", str(project_structure)
        )
...
def test_non_existent_directory_in_path(project_structure):
    with pytest.raises(
        FunctionResolutionError, match="Source file not found"
    ):  # Error will be file not found
        locate_function_source(
            "existingdir.nonexistent.module", "some_func", str(project_structure)
        )


def test_empty_module_path(project_structure):
    with pytest.raises(
        FunctionResolutionError, match="Module path string cannot be empty"
    ):
...
def test_get_function_definitions_single(tmp_path):
    ...
    hat_file = create_dummy_hat_file(
        tmp_path, "single", "fn sum (a:u64 b:u64) u64 { add(a b) }", wrap_main=False
    )
...
def test_get_function_definitions_not_found(tmp_path):
    ...
    hat_file = create_dummy_hat_file(
        tmp_path, "none", "fn add (a:u64 b:u64) u64 { add(a b) }", wrap_main=False
    )
