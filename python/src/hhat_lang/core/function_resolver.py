...
def locate_function_source(
    ...
    if module_path_str == "main":
        if len(module_components) > 1 or module_components[0] != "main":
             raise FunctionResolutionError(f"Invalid module path for 'main': {module_path_str}")
    ...
    for dir_comp in directory_components:
        ...
        if not current_path_check.is_dir() and not current_path_check.exists(): # Check if path exists if it's not the full path yet
...
def get_function_definitions(file_path: str, function_name: str) -> List[FnDef]:
    ...
    visit(ast)
    if not found_defs:
        raise FunctionResolutionError(f"No function definition named '{function_name}' found in {file_path}")
    return found_defs
